...
class TestSearchQueueNotificationWiring:
    """
    Testa que _run_targeted_fetch (search_queue) chama _safe_notify_change
    quando upsert_licitacoes retorna tenders com campos alterados.
    Invoca a função de produção real com todos os I/O externos mockados.
    """

    @pytest.mark.asyncio
    async def test_run_targeted_fetch_dispatches_alert_for_changed_tender(self):
        """
        Integração real: chama _run_targeted_fetch directamente e verifica que
        _safe_notify_change é agendado (via asyncio.ensure_future) para o tender
        cujos campos mudaram.

        Mocks:
          - _get_fetchers: retorna helpers sem HTTP real
          - _fetch_pncp_with_delays: retorna tender conhecido
          - _get_repo: retorna mock upsert que devolve 1 changed_tender
          - _get_pool: retorna mock get_pool
          - cache_scheduler._safe_notify_change: captura chamadas
        """
        import asyncio
        from app.services.search_queue import SearchParams, _run_targeted_fetch

        params = SearchParams(
            q="material", uf=None,
            modal_pncp=(6,), modal_dados=(6,),
            data_ini_iso="2024-01-01", data_fim_iso="2024-12-31",
        )

        changed_tender_fixture = {
            "numero":   "SQ-REAL-001",
            "id":       "SQ-REAL-001",
            "situacao": "encerrado",
        ...
        # Mock upsert_licitacoes devolve 1 changed_tender
        mock_upsert = AsyncMock(return_value=(0, 1, [changed_tender_fixture]))

        # Mock get_pool devolve pool (não importa o conteúdo — upsert é mockado)
        mock_pool       = MagicMock()
        mock_get_pool_fn = AsyncMock(return_value=mock_pool)

        # Mock _enrich_licitacoes: identidade (retorna items sem alteração)
        async def _mock_enrich(items, **kw):
            return items

        # Captura chamadas a _safe_notify_change
        notify_calls: list[dict] = []

        async def _fake_notify(tender: dict) -> None:
            notify_calls.append({"numero": tender.get("numero"), "situacao": tender.get("situacao")})

        # Tuple que _get_fetchers() retorna (a função usa _enrich_licitacoes e _fmt_pncp_date)
        fetchers_tuple = (
            AsyncMock(),           # _fetch_pncp_all_pages_with_cap (não usado directamente)
            AsyncMock(),           # _fetch_dados_all_pages_with_cap (não usado directamente)
            _mock_enrich,          # _enrich_licitacoes — USADO em linha 278
            lambda x: x,           # _fmt_pncp_date — USADO para formatar datas
            {},                    # _pncp_headers
            (6,),                  # _MODALIDADES_PADRAO
            (6,),                  # _MODALIDADES_DADOSABERTOS
            "https://example.com", # _DADOSABERTOS_URL
        )

        with (
            patch("app.services.search_queue._get_fetchers", return_value=fetchers_tuple),
            patch("app.services.search_queue._get_repo",     return_value=mock_upsert),
            patch("app.services.search_queue._get_pool",     return_value=mock_get_pool_fn),
            # _fetch_pncp_with_delays é chamado directamente (não via _get_fetchers)
            patch("app.services.search_queue._fetch_pncp_with_delays",
                  AsyncMock(return_value=([changed_tender_fixture], False))),
            patch("app.services.cache_scheduler._safe_notify_change",
                  side_effect=_fake_notify),
        ):
            result = await _run_targeted_fetch(params)
            # Dá um ciclo do event-loop para que ensure_future possa executar
            await asyncio.sleep(0)

        assert result == 1, f"Esperado 1 upserted, got {result}"
        assert len(notify_calls) == 1, (
            f"_safe_notify_change deve ser chamado 1× para o tender alterado; "
            f"chamadas: {notify_calls}"
        )
        assert notify_calls[0]["numero"]   == "SQ-REAL-001"
        assert notify_calls[0]["situacao"] == "encerrado"

    @pytest.mark.asyncio
    async def test_run_targeted_fetch_no_alert_when_no_changed_tenders(self):
        """
        Quando upsert_licitacoes retorna lista vazia de changed_tenders (somente
        inserções ou sem mudanças rastreadas), nenhum alerta é agendado.
        """
        import asyncio
        from app.services.search_queue import SearchParams, _run_targeted_fetch
        ...
        item = {"numero": "SQ-NEW-001", "id": "SQ-NEW-001", "situacao": "aberto"}
        mock_upsert      = AsyncMock(return_value=(1, 0, []))   # apenas inserção, sem changed
        mock_pool        = MagicMock()
        mock_get_pool_fn = AsyncMock(return_value=mock_pool)

        async def _mock_enrich(items, **kw): return items

        fetchers_tuple = (
            AsyncMock(), AsyncMock(), _mock_enrich, lambda x: x, {}, (6,), (6,), "",
        )

        notify_called = []

        async def _fake_notify(t): notify_called.append(t)
        ...
        with (
            ...
            patch("app.services.search_queue._get_pool",     return_value=mock_get_pool_fn),
            patch("app.services.search_queue._fetch_pncp_with_delays",
                  AsyncMock(return_value=([item], False))),
            patch("app.services.cache_scheduler._safe_notify_change",
                  side_effect=_fake_notify),
        ):
            result = await _run_targeted_fetch(params)
        ...
        assert result == 1
        assert notify_called == [], "Nenhum alerta deve ser disparado para inserções sem mudanças"
